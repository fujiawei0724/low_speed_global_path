...
class Config {
 public:
    ...
    // 真实的机器人长度
    constexpr static double robot_length_ = 0.4;

    // 真实机器人宽度
    constexpr static double robot_width_ = 0.4;
    ...
    // 栅格分辨率
    constexpr static double grid_resolution_ = 0.5;
    ...
    // 起点和终点影响范围
    constexpr static double influence_range_ = 8.0;

    // 离障碍物距离最大阈值(路径平滑)
    constexpr static double smooth_max_distance_to_obs_ = 2.5;
    ...
    // 路径平滑权重(曲率)
    constexpr static double smooth_cur_weight_ = 50.0;
    ...
    // global planning max distance to obstacle
    constexpr static double global_planning_max_distance_to_obs_ = 4.0;
